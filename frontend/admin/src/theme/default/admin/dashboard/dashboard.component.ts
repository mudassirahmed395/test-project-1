...
@Component({
  ...
  providers: [...chatConversationComponents]
})
export class DashboardComponent implements OnInit {
  ...
  public vendors: any = [];
  public vendorsyear: any = "2020";
  public vendorList: any;
  public vendorID: any = null;
...
private currency: any;
  ...
  constructor(
    ...
    public translate:TranslateService, private injector: Injector
    ...
    )
   {
    ...
    if(!this.currency) {
      this.layoutSandbox.settings$.subscribe(data => {
        this.currency = data
  ...
  this.vendorsyear=this.currentDate.year.toString();
  ...
  ngOnInit() {
    if(chatConversationComponents?.length > 0) {
      try {
        this.chat = this.injector.get(chatConversationComponents[0]);
        this.chat.vendorList1();
      } catch (e) {
        console.warn('ChatconversationComponent is not available');
        this.chat = undefined;
      }
    }
      ...
      if (data && (this.xvalue1.length === 0)) {
        ...
        this.transactionslist.forEach(element => {
          if (element) {
            ...
            const val = (this.currency.position === 'left' ? ' '+this.currency.symbol : '') + 
            element.orderAmount + 
            (this.currency.position === 'right' ? ' '+this.currency.symbol : '');
            this.resultValue.push(element.ordercount + ',' + ' ' +val)
  ...
  sellerList() {
    ...
    this.subscriptions.push(this.sellerSandbox.getSellerList$.subscribe(datas => {
      if (datas) {
        this.vendorList = datas;
        this.vendorList = datas.filter(item => item.approvalFlag == 1 && item.companyName != null);
        this.vendorID = this.vendorList[0].vendorId.toString();
        this.vendor();
      }
    }))
  }
  ...
  vendor() {
    ...
    params.duration = this.vendorsyear;
    this.dashboardSandbox.vendor(params);
    this.subscriptions.push(this.dashboardSandbox.vendor$.subscribe(data => {
      this.vendors = [];
      if (data?.array) {
          for (let i = 0; i < data.array.length; i++) {
            switch (data.array[i].name) {
              case "productCount":
                this.vendors.push({ name: "Product Count", value: data.array[i].value });
                break;
        
              case "productSoldCount":
                this.vendors.push({ name: "Product Sold Count", value: data.array[i].value });
                break;
        
              case "deliveryCount":
                this.vendors.push({ name: "Delivery Count", value: data.array[i].value });
                break;
        
              case "outOfStockCount":
                this.vendors.push({ name: "Out Of Stock Count", value: data.array[i].value });
                break;
        
              case "pendingStockCount":
                this.vendors.push({ name: "Pending Stock Count", value: data.array[i].value });
                break;
        
              default:
                // Handle unexpected names, if necessary
                break;
            }
  ...
  checkboxweekly(event, item) {
    if (event.target.checked == true) {
      if (this.top10weeklyproductdatalist.length > 2) {
        this.translateName=this.translate.instant('Dashboard.chooseAnyThree');
